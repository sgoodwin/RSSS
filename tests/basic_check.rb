...
def check_home
  res = Net::HTTP.get_response URI.parse("http://0.0.0.0:8080");
  puts res.body;
end
check_home
...
def check_new_user
  ...
  puts "\r\n\r\nNew User:";
  res = Net::HTTP.get_response URI.parse("http://0.0.0.0:8080/newuser.json");
  parse_json(res.body);
end
#check_new_user
...
def check_check_user
  ...
  puts "\r\n\r\nCheck User:";
  res = Net::HTTP.get_response URI.parse("http://0.0.0.0:8080/check.json?key=1");
  parse_json(res.body);
end
check_check_user
...
def check_user_invalid
  ...
  res = Net::HTTP.get_response URI.parse("http://0.0.0.0:8080/check.json?key=defdoesnotexist");
  parse_json(res.body);
end
check_user_invalid
...
def check_new_feed
  ...
  res = Net::HTTP.post_form(URI.parse('http://0.0.0.0:8080/feed.json'),{'key'=>'1', 'title'=>'Denver Skate Shop', 'rssURL'=>'http://denverskateshop.blogspot.com/feeds/posts/default', 'htmlURL'=>'http://denverskateshop.blogspot.com/'})
  parse_json(res.body);
end
#check_new_feed
...
def check_new_feed_invalid
  ...
  res = Net::HTTP.post_form(URI.parse('http://0.0.0.0:8080/feed.json'),{'key'=>'defdoesnotexist', 'title'=>'Denver Skate Shop', 'rssURL'=>'http://denverskateshop.blogspot.com/feeds/posts/default', 'htmlURL'=>'http://denverskateshop.blogspot.com/'})
  puts res.body;
end
...
def check_destroy_feed
  puts "\r\n\r\nDestroy Feed:"
  uri = URI.parse("http://0.0.0.0:8080/feed/6")
  Net::HTTP.start(uri.host, uri.port) do |http|
    headers = {'Content-Type' => 'application/x-www-form-urlencoded'}
    put_data = "key=1"
...
def check_feedlist_opml
  ...
  res = Net::HTTP.get_response URI.parse("http://0.0.0.0:8080/feedlist.opml?key=1");
  return res
end
...
def check_feedlist_json_conditional
  ...
  uri = URI.parse("http://0.0.0.0:8080/feedlist.json?key=1");
  Net::HTTP.start(uri.host, uri.port) do |http|
...
def check_feedlist_json
  # GET feedlist
  puts "\r\n\r\nJSON Feed List:";
  res = Net::HTTP.get_response URI.parse("http://0.0.0.0:8080/feedlist.json?key=1");
  parse_json(res.body);
end
check_feedlist_json
...
def check_feedlist_invalid
  ...
  res = Net::HTTP.get_response URI.parse("http://0.0.0.0:8080/feedlist.opml?key=defdoesnotexist");
  puts res.body;
end
